...
class MCTree:
    ...
    def train(self, X, Y):
        for n in self.tree.iterNodes():
            ...
            thisX = X[mask]
            thisY = zeros(mask.sum())
            
            # Assign -1 to left subtree labels and +1 to right subtree labels
            for label in leftLabels:
                thisY[Y[mask] == label] = -1
            for label in rightLabels:
                thisY[Y[mask] == label] = 1
    ...
    def predict(self, X):
        ...
        while not node.isLeaf:
            # Get prediction from node's classifier
            pred = node.getNodeInfo().predict(X.reshape(1, -1))[0]
            # Go left if prediction is negative, right if positive
            if pred < 0:
                node = node.getLeft()
            else:
                node = node.getRight()
